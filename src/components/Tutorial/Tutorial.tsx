...
import type { TutorialProps } from '../../types/components'
import type { Theme } from '../../types/data'

// Constants
const CENTERED_CONTENT_STYLE: React.CSSProperties = { textAlign: 'center' }
const ACCENT_COLOR = '#FFB039'

// Helper function to create centered content
const createCenteredContent = (text: string) => (
  <div>
    <p style={CENTERED_CONTENT_STYLE}>{text}</p>
  </div>
)

// Helper function to create step title
const createStepTitle = (text: string) => <h2>{text}</h2>

// Helper function to get theme classes
const getThemeClasses = (theme: Theme, baseClasses = '') => {
  const themeClasses = theme === 'dark' ? 'text-white fill-white' : 'text-black fill-black'
...
const TutorialCard = ({
  ...
  <div className={clsx('tutorial-card flex flex-col items-center justify-center', className)}>
    <div className='image-card h-32 w-32'>{icon}</div>
    <p className={getThemeClasses(theme, 'mt-4 text-center')}>
      {label.split('\n').map((line, i) => (
...
// SVG Icon Components
const DesktopRotateIcon = ({ theme }: { theme: Theme }) => (
  <svg width='100%' height='100%' viewBox='0 0 78 121' fill='none' xmlns='http://www.w3.org/2000/svg'>
    ...
    <path
      ...
      className={getThemeClasses(theme, 'fill')}
    />
  </svg>
)

const DesktopZoomIcon = ({ theme }: { theme: Theme }) => (
  <svg width='100%' height='100%' viewBox='0 0 78 121' fill='none' xmlns='http://www.w3.org/2000/svg'>
    <rect x='34' y='16' width='10' height='26' fill={ACCENT_COLOR} />
    <path
      fillRule='evenodd'
      clipRule='evenodd'
...
// Mobile SVG Icons (keeping original paths for now due to complexity)
const MobileRotateIcon = ({ theme }: { theme: Theme }) => (
...
const MobileZoomIcon = ({ theme }: { theme: Theme }) => (
...
export default function Tutorial(props: TutorialProps) {
    ...
    () => [
      {
        placement: 'center',
        target: 'body',
        title: createStepTitle('Tutorial'),
        content: createCenteredContent(
          'Selamat Datang di Virtual Tour FT UGM, Gunakanlah komputer untuk pengalaman pengguna yang lebih baik.',
        ),
      },
      {
        target: '.show-tooltip',
        title: createStepTitle('Bangunan'),
        content: createCenteredContent(
          'Tombol ini digunakan untuk melihat nama bangunan yang terdapat di FT UGM. Anda bisa menekan objek bangunan untuk melihat informasi yang lebih rinci',
        ),
      },
      {
        target: '.jelajah-teknik',
        title: createStepTitle('Jelajah Teknik'),
        content: createCenteredContent(
          'Tombol ini digunakan untuk menampilkan halaman Jelajah Teknik yang akan membantu Anda dalam menjelajahi panorama teknik dengan lebih baik.',
        ),
      },
      {
        target: '.faq',
        title: createStepTitle('Frequently Asked Questions'),
        content: createCenteredContent('Tombol ini digunakan untuk menampilkan halaman FaQ.'),
      },
      {
        target: '.night-mode',
        title: createStepTitle('Tema'),
        content: createCenteredContent('Tombol ini digunakan merubah tema halaman menjadi gelap atau terang.'),
      },
      {
        target: '.tour-music',
        title: createStepTitle('Musik latar belakang'),
        content: createCenteredContent(
      ...
      {
        target: '.about-us',
        title: createStepTitle('Tentang Kami'),
        content: createCenteredContent('Tombol ini digunakan untuk menampilkan halaman Tentang Virtual Tour FT UGM.'),
      },
      {
        target: 'body',
        placement: 'center',
        ...
        content: (
          <div className='tutorial-container flex h-full w-full flex-col items-center justify-center'>
            <div className='tutorial-section my-4 flex w-full flex-col items-center justify-evenly sm:flex-row'>
              <TutorialCard
                icon={<DesktopRotateIcon theme={navigation.theme} />}
                label='Memutar\nKamera'
                theme={navigation.theme}
                className='sm:mb-0'
              />
              <TutorialCard
                icon={<DesktopZoomIcon theme={navigation.theme} />}
                label='Mengatur Jarak\nKamera'
                theme={navigation.theme}
              />
            </div>
          </div>
      ...
      },
      {
        ...
        content: (
          <div className='tutorial-container flex h-full w-full flex-col items-center justify-center'>
            <div className='tutorial-section my-8 flex w-full flex-col items-center justify-evenly sm:flex-row'>
              <TutorialCard
                icon={<MobileRotateIcon theme={navigation.theme} />}
                label='Memutar\nKamera'
                theme={navigation.theme}
                className='mb-4 sm:mb-0'
              />
              <TutorialCard
                icon={<MobileZoomIcon theme={navigation.theme} />}
                label='Mengatur Jarak\nKamera'
                theme={navigation.theme}
              />
            </div>
      ...
      },
      {
        placement: 'center',
        target: 'body',
        title: createStepTitle('Navigasi pada Model 3 Dimensi'),
        content: (
          <div className='mt-4 flex flex-col items-center justify-center'>
            <svg width='128' height='128' viewBox='0 0 463 479' fill='none' xmlns='http://www.w3.org/2000/svg'>
              <path
                ...
                className={getThemeClasses(navigation.theme, 'fill')}
              />
              <path
              ...
              />
              {[
                [64.3334, 80.3334, 96.1667, 112.167],
                [128, 80.3334, 159.833, 112.167],
                [191.667, 80.3334, 223.5, 112.167],
                [64.3334, 159.917, 96.1667, 191.75],
                [128, 159.917, 159.833, 191.75],
                [191.667, 159.917, 223.5, 191.75],
                [64.3334, 239.5, 96.1667, 271.333],
                [128, 239.5, 159.833, 271.333],
                [191.667, 239.5, 223.5, 271.333],
                [64.3334, 319.083, 96.1667, 350.917],
                [128, 319.083, 159.833, 350.917],
                [191.667, 319.083, 223.5, 350.917],
                [303.083, 159.917, 334.917, 191.75],
                [366.75, 159.917, 398.583, 191.75],
                [303.083, 239.5, 334.917, 271.333],
                [366.75, 239.5, 398.583, 271.333],
                [303.083, 319.083, 334.917, 350.917],
                [366.75, 319.083, 398.583, 350.917],
              ].map(([x, y, x2, y2], i) => (
                <path key={i} d={`M${x} ${y}H${x2}V${y2}H${x}V${y}Z`} fill={ACCENT_COLOR} />
              ))}
            </svg>
            <p className={getThemeClasses(navigation.theme, 'mt-6 w-4/5 text-center')}>
      ...
      },
      {
        target: '.tutorial',
        title: createStepTitle('Tutorial'),
        content: createCenteredContent('Tombol ini digunakan untuk menampilkan tutorial Kembali.'),
      },
    ],
    [navigation.theme],
  )
  ...
  const joyrideStyles = useMemo<Styles>(
    () => ({
      options: {
        zIndex: 999999999,
        width: 700,
        backgroundColor: isDark ? '#121212' : '#fff',
        primaryColor: isDark ? '#fff' : '#121212',
        textColor: isDark ? '#fff' : '#121212',
        arrowColor: isDark ? '#121212' : '#fff',
        margin: '40px',
      },
      buttonNext: {
        borderRadius: 4,
  ...
  return (
    <Joyride
      ...
      } as React.ComponentProps<typeof Joyride>)}
    />
  )
}
