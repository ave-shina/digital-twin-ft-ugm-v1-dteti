...
import { MapInteractionCSS, type MapInteractionValue } from 'react-map-interaction'
import { Stage, Layer, Image as KonvaImage, Circle } from 'react-konva'
import type Konva from 'konva'

import { useAppSelector, useAppDispatch } from 'redux/hooks'
import { toggleLocation, setContent, setMapTourMessage, setMapLandmarkMessage } from 'redux/navigation'

import { useRouter } from 'next/router'
import { withImgixParams } from '@/lib/prismic/imgix'
import type { MapProps } from '../../types/components'
import type { MapInformation } from '../../types/data'
...
function Map(props: MapProps) {
  ...
  for (let i = 0; i < mapInformation.length; i++) {
    map.push({
      ...
      isCrooked: (mapInformation[i] as MapInformation & { isCrooked?: boolean }).isCrooked,
      isRoute: true,
    })
  }
  ...
  useEffect(() => {
    ...
    // Resize via imgix untuk Prismic (Cloudinary dulu memakai transformasi URL).
    img.src = withImgixParams(mapImage.url, { w: 1000, auto: 'format', q: 'auto' })
  ...
  const [value, setValue] = useState<MapInteractionValue | null>(null)
  ...
  const handleMouseEnter = (e: Konva.KonvaEventObject<Event>, name: string) => {
    const stage = e.target.getStage()
    const container = stage?.container()
    if (container) container.style.cursor = 'pointer'
    setcircleName(name)
  }
  const handleMouseLeave = (e: Konva.KonvaEventObject<Event>, name: string) => {
    const stage = e.target.getStage()
    const container = stage?.container()
  ...
  function handleClick(isRoute: boolean, name: string) {
    if (isRoute) {
      setCurrentScene(name)
      setOpenPanorama(true)
    } else {
      dispatch(setContent('landmark'))
