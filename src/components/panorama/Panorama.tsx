...
import type { Pannellum as PannellumInstance } from '@georgedrpg/pannellum-react-next'
...
import type { PanoramaProps } from '../../types/components'
...
export default function Panorama(props: PanoramaProps) {
  ...
  const panImage = useRef<PannellumInstance>(null)

  // Membuat Tombol untuk navigasi antar lokasi
  const hotspotIcon = useCallback((hotSpotDiv: HTMLDivElement) => {
  ...
  const imageUrl = useMemo(() => {
    if (!sceneObject?.scenePanoImg) return undefined
    // Resize panorama via imgix untuk Prismic (dulu transformasi URL Cloudinary).
    return withImgixParams(sceneObject.scenePanoImg.url, { w: 4000, auto: 'format', q: 'auto' }) || undefined
  }, [sceneObject?.scenePanoImg])
  ...
  const hotspotsWithHandlers = useMemo(() => {
    ...
    return sceneObject.hotSpotsArr.map((hotSpot) => ({
      ...hotSpot,
      handleClick: (_evt: unknown, _name: unknown) => {
